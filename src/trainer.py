...
class RFFTrainer(mt.BaseTrainer):
    def __init__(self):
        ...
        if (not self.d1 is None) and (not self.d2 is None):
            self.models['C'] = self.eval_model(in_channels=self.nc, out_channels=self.class_num, d1=self.d1, d2=self.d2,
                                               z_dim=self.z_dim).to(self.device)
        else:
            self.models['C'] = self.eval_model(in_channels=self.nc, out_channels=self.class_num, z_dim=self.z_dim).to(
                self.device)

        self.optims['C'] = torch.optim.Adam(
            self.models['C'].parameters(), lr=1e-4, betas=(0.5, 0.99))

        self.datasets['train'] = RFdataset(device_ids=self.train_devices, test_ids=self.train_ids,
                                           rand_max_SNR=self.train_snr)
    ...
    def eval(self, epoch, eval_dataset='open'):
        ...
        eer, roc_auc, thresh = get_auc_eer(intra_dist, inter_dist, plot_roc=True,
                                           filename='./plots/{}_{}_roc.png'.format(self.flag, eval_dataset))
        is_best = False
